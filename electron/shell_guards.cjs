...
function buildContentSecurityPolicy({ apiOrigin, scriptHashes = [] } = {}) {
  ...
  // "Refused to connect" and hangs both world entry and a live graphics-settings rebuild on the
  // desktop shell (the only host that ships a CSP). data: is a self-contained inline scheme, not
  // a network origin, so unlike a remote host it opens no exfiltration path; img-src already
  // lists it for the same texture reason.
  const connectSrc = [
