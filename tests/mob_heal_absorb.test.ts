...
describe('mob heal-absorb ("Grave Blight")', () => {
  ...
  it('devours healing up to its budget, then leaves the rest, depleting the shield', () => {
    ...
    // A 200-point heal eats the remaining 70 and 130 survives; the shield zeroes.
    expect((sim as any).consumeHealAbsorb(p, 200)).toBe(130);
    // Depleted, but zeroed in place rather than spliced out (it still expires on
    // its own `remaining` like any other aura).
    const shield = p.auras.find((a) => a.kind === 'heal_absorb');
