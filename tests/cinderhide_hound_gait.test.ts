// Regression pin: the Cinderhide Hound's yaw: Math.PI facing fix (copied from
// the Veil-Wraith Courser's genuinely flat-180 rig) was itself the bug. A
// live orientation check (mount_visuals.test.ts's yaw pin covers that half)
// found the hound's raw mesh faces along X, not Z, exactly like the Nightprowl
// Panther: scripts/bake_mount_gaits.mjs needs bodyAxis: 'x' for this rig too,
// or the leg-swing math (which rotates about world X, the axis that moves a
// Z-forward foot fore-aft but only ever moves an X-forward foot sideways)
// swings the legs left-right instead of forward-back. Verified directly off
// the baked animation data (not a screenshot), the same method
// nightprowl_panther_gait.test.ts uses: the front-left foot's displacement
// across the Walk cycle must be dominant on X, matching the rig's own real
// fore-aft axis.
...
describe('cinderhide_hound gait direction (facing-fix follow-up)', () => {
  it('swings the front-left foot fore-aft along X, not sideways along Z', async () => {
    await MeshoptDecoder.ready;
    const io = new NodeIO()
      .registerExtensions(ALL_EXTENSIONS)
      .registerDependencies({ 'meshopt.decoder': MeshoptDecoder });
    const doc = await io.read('public/models/mounts/cinderhide_hound.glb');
    ...
    expect(
      Math.abs(d[0]),
      'X displacement must dominate Z (a sideways swing is the bug this pins)',
