...
//
// `IWorld` (src/world_api.ts, 218 members) is the ONE seam render/ui depend
...
// it lands the matching edit here in the SAME commit. The count pins (218 / 60 / 158)
...
// The 218 members of `interface IWorld`, in interface order (world_api.ts).
// Partition: 60 `data` + 158 `method` (read-returning + command-void + async).
...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  ...
  it('the full sorted member set is exactly the pinned 218', () => {
  ...
  it('the sorted method-kind set is exactly the pinned 158', () => {
...
//   (4) their union, sorted, equals the pinned 218-name IWORLD_MEMBERS set (a member
