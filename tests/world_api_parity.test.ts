...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  it('pins total / data / method counts', () => {
    ...
    // payload on demand: it has no snapshot key), leaving 288. The Thornhollow
    // Fields release adds the four battleground facet members on top of
    // that base: the bgInfo data member plus the bgQueueJoin / bgQueueLeave /
    // bgFlagAction commands, leaving 293. This branch's commission order
    // board (issue #1298) adds commissionOrders (data) plus
    // openCommissionOrder/cancelCommissionOrder/acceptCommissionOrder/
    // deliverCommissionOrder (methods), leaving 298.
    expect(IWORLD_MEMBERS.length).toBe(298);
    expect(DATA_MEMBERS.length).toBe(76);
    expect(METHOD_MEMBERS.length).toBe(222);
  });
...
describe('W1: aggregate IWorld member set equals the disjoint union of the facets', () => {
  ...
  it('the facet union equals the pinned IWORLD_MEMBERS set', () => {
    ...
    expect(union.length, 'union size before dedup (catches a duplicated member)').toBe(298);
    expect(new Set(union).size, 'union size after dedup (catches a duplicated member)').toBe(298);
