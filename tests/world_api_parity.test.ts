...
//
// `IWorld` (src/world_api.ts, 164 members) is the ONE seam render/ui depend
...
// it lands the matching edit here in the SAME commit. The count pins (164 / 46 / 118)
...
// The 164 members of `interface IWorld`, in interface order (world_api.ts:342-509).
// Partition: 46 `data` + 118 `method` (read-returning + command-void + 3 async).
...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  it('pins total / data / method counts', () => {
    ...
    expect(METHOD_MEMBERS.length).toBe(118);
  });
  ...
  it('the full sorted member set is exactly the pinned 164', () => {
...
//   (4) their union, sorted, equals the pinned 164-name IWORLD_MEMBERS set (a member
...
describe('W1: aggregate IWorld member set equals the disjoint union of the 21 facets', () => {
  ...
  it('the union of the 21 facets equals the pinned 164-member IWORLD_MEMBERS set', () => {
