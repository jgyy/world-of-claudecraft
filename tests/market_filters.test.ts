...
describe('World Market filters', () => {
  ...
  it('routes bags to their own bucket, never to Other or any other specific bucket', () => {
    // Bags are real, listable items (src/sim/market.ts only refuses kind:'quest' and
    // noMarketList/soulbound items at list time) and have their own dedicated 'bag'
    // bucket (see the "browses bags as their own category" case above), so they must
    // never surface under 'other' or leak into any of the more specific buckets.
    const mixed = [
      'linen_pouch',
      'wolfhide_satchel',
      'keen_dirk',
    ...
    expect(filterIds(mixed, { itemType: 'bag' })).toEqual(['linen_pouch', 'wolfhide_satchel']);
    expect(filterIds(mixed, { itemType: 'other' })).toEqual([]);
