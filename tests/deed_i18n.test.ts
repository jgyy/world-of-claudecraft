...
describe('deed_i18n English resolution', () => {
  ...
  it('manifests one row per name and desc plus one per title reward', () => {
    ...
    // 240 deeds x (name + desc) + the 31 shipped title rewards (both counts
    // pinned by tests/deeds_content.test.ts): the Drakelands brood pair, the
    // four Thornhollow Fields battleground deeds, plus the Rift coverage pair
    // (dgn_rift, dgn_rift_s_rank; neither carries a title reward).
    expect(manifest.length).toBe(240 * 2 + 31);
