...
describe('item-art consistency accepted-art provenance', () => {
  ...
  it('pins the final 817-item visual audit, Heroic accounting, and evidence digests', () => {
    ...
    expect(verdict.auditScope).toMatchObject({
      ...
      itemArtFilesReviewed: 824,
      liveItemDefinitions: 839,
    ...
    expect(verdict.evidence.catalog).toEqual({
      ...
      sha256: 'e92eeb29b27e8e40f241d0b0b2db1e6fafa8e3df53a608f76e8e14e937dadef3',
      bytes: 452_340,
    });
    ...
    expect(verdict.evidence.sheetSetSha256).toBe(
      '377e99730f39d6fdbd14b159bf499bcce5a54fd23160e9bebedd5ce7cb01d85f',
    );
    ...
    expect(verdict.evidence.shippingCatalogSha256).toBe(
      '962cbaea18cdfd2beee5fb4fb10195aaf8e141f7bd827d5ca9556d03ce23fcfb',
    );
  ...
  it('keeps an exact, non-dangling supersession graph and one current mapping owner', () => {
    ...
    expect(allCurrentOwnerIds).toHaveLength(824);
    expect(new Set(allCurrentOwnerIds).size).toBe(824);
  ...
  it('keeps the full 817-icon catalog owned, decodable, opaque, budgeted, and unique', async () => {
    ...
    if (ownerIds.length !== 824) violations.push(`mapping owner count: ${ownerIds.length} != 824`);
    if (fileIds.length !== 824) violations.push(`shipping WebP count: ${fileIds.length} != 824`);
    for (const id of ids) {
