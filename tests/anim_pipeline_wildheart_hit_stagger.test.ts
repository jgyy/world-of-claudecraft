...
describe('TRIPO_BIPED_FULL_RIG hit-reaction stagger (issue #2889 round 2)', () => {
  ...
  it('wires a matching animUrls entry onto every TRIPO_BIPED_FULL_RIG consumer', () => {
    // mob_wildheart_stalker, mob_wildheart_ravager, mob_wildheart_hexcaller,
    // mob_wildheart_beastmaster, and mob_wildheart_high_priest were each split onto their
    // own clips const (WILDHEART_STALKER / WILDHEART_RAVAGER / WILDHEART_HEXCALLER /
    // WILDHEART_BEASTMASTER / WILDHEART_HIGH_PRIEST) for their own attack/cast clip; all
    // five spread TRIPO_BIPED_FULL_RIG (including the shared Hit_Stagger hit array), so they
