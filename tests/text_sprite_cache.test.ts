...
describe('text_sprite_cache: the bound and its eviction', () => {
  it('ships the budget it documents', () => {
    ...
    // 416 -> 800: the guild roster ladder (a guild can now seat 500) took the
    // ally-name term from 150 to 550 and the derived worst case to 785.
    expect(TEXT_SPRITE_LIMIT).toBe(800);
  });
  ...
  it('stays above the largest label set one redraw can ask for', () => {
    ...
    // every ladder page), not the base: a 500-seat guild entirely online is the
    // set one redraw can ask for.
    ...
    expect(allyNames, 'ally names: the friend cap plus the largest guild roster').toBe(550);
    ...
    expect(
      worstCase,
      'the header quotes 785; a SMALLER total means a term broke',
    ).toBeGreaterThanOrEqual(785);
    expect(
      TEXT_SPRITE_LIMIT,
