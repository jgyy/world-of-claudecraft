...
await page.evaluate(({ RUN_SPEED }) => {
  ...
  const L = 120, R = 1520, T = 130, B = 770;
  const xMin = 3, xMax = 9, yMax = 11;
  ...
  c.fillStyle = '#9fb0c3'; c.fillText('mob base move speed (yd/s)', (L + R) / 2 - 110, B + 70);
  ...
  const plot = (fn, color, label, dy) => {
    ...
    for (let b = xMin; b <= xMax; b += 0.1) {
      const x = sx(b), y = sy(fn(b)); first ? (c.moveTo(x, y), first = false) : c.lineTo(x, y);
    }
    c.stroke();
    c.fillStyle = color; c.fillText(label, R - 360, sy(fn(xMax)) + dy);
  };
