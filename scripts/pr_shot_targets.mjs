...
// Poll up to ~6s for the full-screen #loading-screen curtain (main.ts) to
// finish its OWN fade-out (the `visible` class removal, on a timer independent
// of enterOfflineGame's settle wait). Under headless SwiftShader contention
// deep in a long capture session, a scripted position jump right after entry
// can still be mid-curtain: the windows this then opens render underneath it
// (pollForSize on them passes) but the curtain paints over everything by
// screenshot time, an intermittent all-black/logo shot instead of the game.
async function pollForNoLoadingCurtain(page, attempts = 12, intervalMs = 500) {
  for (let i = 0; i < attempts; i++) {
