...
**Proximity voice chat.** The Game includes an optional, opt-in proximity voice channel (Settings > Audio). If you enable it, your microphone audio is sent directly to a small number of nearby opted-in players over a peer-to-peer WebRTC connection, never through our servers. Enabling a direct peer-to-peer connection necessarily exposes your public IP address to the other player in that connection, the same way it would in any other peer-to-peer voice or video call; we do not otherwise store or relay the audio itself. Our server only decides, based on your in-world position, which nearby opted-in players to pair you with, and relays the connection-setup (signaling) messages needed to establish each peer-to-peer link. Voice chat is off by default and only requests microphone access once you turn it on; turning it off immediately stops new pairings.
