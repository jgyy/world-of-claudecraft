...
export class AurasPainter {
  ...
  paint(state: AurasState): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      rec.seen = this.frame;
      // Cancel affordance: only when this painter has an onCancel dep (the player buff bar)
      // and the aura is a helpful buff, never a debuff. Read live by the contextmenu closure.
      rec.auraId = s.key;
      rec.cancelable = !!this.deps.attachCancel && !s.isDebuff;
