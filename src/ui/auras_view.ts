...
export interface AuraSlotState {
  ...
  remaining: number;
  /** Whether this aura is the player's own cancelable buff (mode 'all', not a debuff):
  ...
  return {
    tick(entity: AurasEntityInput): AurasState {
      ...
      for (const a of entity.auras) {
        ...
        // The buff bar (mode 'all', the player's own auras) offers right-click-cancel;
        ...
        slot.cancelable = mode === 'all' && !debuff;
