// The empty Entity the wire mirror grows a fresh row from (ClientWorld
// applySnapshot: every field the renderer and HUD read must exist before the
// first wire delta lands, so a stale server never leaves a hole). Extracted
// from src/net/online.ts as-is: a pure constructor with no client state, so
// the coordinator only composes it.
