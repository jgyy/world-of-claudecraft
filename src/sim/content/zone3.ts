...
export const ZONE3_NPCS: Record<string, NpcDef> = {
  ...
  astronomer_veyra: {
    id: 'astronomer_veyra', name: 'Astronomer Veyra', title: 'Keeper of the Highwatch Observatory',
    pos: { x: 18, z: 648 }, facing: -2.2, color: 0x4b3b8f,
    questIds: [
    ...
    greeting: 'A wyrm-star is rising over the Sanctum, $N, and I am the only one on this wall who can read what it portends — if the peaks would only let me keep the sky in my glass.',
  },
};
...
export const ZONE3_QUESTS: Record<string, QuestDef> = {
  ...
  // --- The Highwatch Observatory: Astronomer Veyra's star-charting chain (all-kill, zone3 camps) ---
  q_obs_sightlines: {
    id: 'q_obs_sightlines', name: 'A Clear Horizon',
    giverNpcId: 'astronomer_veyra', turnInNpcId: 'astronomer_veyra',
    text: "A telescope is only as good as the dark it stares into, $N, and the ridge below my dome is anything but dark — every night the ridge stalkers prowl the scree and set the picket-hounds baying, and a startled astronomer is a blurred plate. I cannot chart the wyrm-star with one hand on a crossbow. Cull twelve of the brutes off the ridge and give me one quiet night to take my readings.",
    completionText: "One quiet night, and the plate came out sharp as glass. The wyrm-star sits exactly where the old charts said it would not. That alone is worth the powder, $N.",
    objectives: [{ type: 'kill', targetMobId: 'ridge_stalker', count: 12, label: 'Ridge Stalker slain' }],
    xpReward: 1500, copperReward: 520, itemRewards: {},
    minLevel: 13,
  },
  q_obs_burrowlight: {
    id: 'q_obs_burrowlight', name: 'Light in the Burrows',
    giverNpcId: 'astronomer_veyra', turnInNpcId: 'astronomer_veyra',
    ...
    completionText: "The west has gone dark at last, and the faint stars behind the wyrm-star have come out of the glare. You have given me back half my sky, $N.",
    objectives: [{ type: 'kill', targetMobId: 'deeprock_kobold', count: 12, label: 'Deeprock Tunneler slain' }],
    xpReward: 1550, copperReward: 540, itemRewards: {},
    minLevel: 13,
  },
  q_obs_cragmaw: {
    id: 'q_obs_cragmaw', name: 'The Tripod-Breaker',
    giverNpcId: 'astronomer_veyra', turnInNpcId: 'astronomer_veyra',
    text: "Some beast came up the scree in the dark and put a shoulder through my mounted glass — splintered the tripod, bent the great brass tube I waited two years to have hauled up the pass. The tracks are a bear's, and the wardens have a name for the old bull that ranges that slope: Cragmaw. I cannot replace that instrument, $N. Put the beast down before it finishes what it started.",
    completionText: "So that is the brute. Look at the reach on it — no wonder my tripod folded. The glass can be straightened; with Cragmaw dead, it will not be knocked down twice.",
    objectives: [{ type: 'kill', targetMobId: 'old_cragmaw', count: 1, label: 'Old Cragmaw slain' }],
    xpReward: 1700, copperReward: 640, itemRewards: {},
    requiresQuest: 'q_obs_sightlines',
    minLevel: 14,
  },
  q_obs_stormglare: {
    id: 'q_obs_stormglare', name: 'Glare off the Crags',
    giverNpcId: 'astronomer_veyra', turnInNpcId: 'astronomer_veyra',
    text: "The Stormcrag elementals have woken in numbers, and every time one of them discharges, the whole sky over the western crags whites out and my night-eye is ruined for an hour. I have lost three nights of the wyrm-star's transit to their lightning. Scatter ten of them off the high crags, $N — I need an unbroken hour of true dark to time the thing's passage.",
    completionText: "A whole hour without a single flash. I timed the transit clean from limb to limb — and the wyrm-star is moving faster than anything that far out has any right to. That troubles me more than the ogres do.",
    objectives: [{ type: 'kill', targetMobId: 'stormcrag_elemental', count: 10, label: 'Stormcrag Elemental slain' }],
    xpReward: 1750, copperReward: 660, itemRewards: {},
    requiresQuest: 'q_obs_burrowlight',
    minLevel: 15,
  },
  q_obs_shardlord: {
    id: 'q_obs_shardlord', name: 'The Living Lantern',
    giverNpcId: 'astronomer_veyra', turnInNpcId: 'astronomer_veyra',
    ...
    completionText: "The west has stopped flickering. With Kazzix gone the deep field came out black and still, and the plates I pulled tonight are the finest I have ever taken on this peak. Whatever is coming, I will see it first.",
    objectives: [{ type: 'kill', targetMobId: 'shardlord_kazzix', count: 1, label: 'Shardlord Kazzix slain' }],
    xpReward: 1950, copperReward: 820, itemRewards: {},
    requiresQuest: 'q_obs_stormglare',
    minLevel: 16,
    suggestedPlayers: 3,
  },
  q_obs_ogrefires: {
    id: 'q_obs_ogrefires', name: 'Bonfires on the Foothills',
    giverNpcId: 'astronomer_veyra', turnInNpcId: 'astronomer_veyra',
    ...
    completionText: "The eastern fires are guttering and the rising-stars have come back to me. I have my reference points on both horizons now — I can finally pin the wyrm-star's true track across the whole vault.",
    objectives: [{ type: 'kill', targetMobId: 'thornpeak_ogre', count: 12, label: 'Thornpeak Ogre slain' }],
    xpReward: 1850, copperReward: 700, itemRewards: {},
    requiresQuest: 'q_obs_sightlines',
    minLevel: 15,
  },
  q_obs_crushers: {
    id: 'q_obs_crushers', name: 'Stokers of the Pyres',
    giverNpcId: 'astronomer_veyra', turnInNpcId: 'astronomer_veyra',
    text: "Thin the rank-and-file and the fires only flare higher, because it is the Thornpeak crushers who feed them — each one drags whole trees to the pyres to keep the foothills lit through the night. As long as the crushers stoke them, the eastern sky stays bright. They are elite and they hit like falling rock, $N; take companions and break eight of them.",
    completionText: "Eight crushers down, and the great pyres have burned to embers with no one left to feed them. The eastern dark is mine for as long as it takes to finish the survey.",
    objectives: [{ type: 'kill', targetMobId: 'ogre_crusher', count: 8, label: 'Thornpeak Crusher slain' }],
    xpReward: 1950, copperReward: 780, itemRewards: {},
    requiresQuest: 'q_obs_ogrefires',
    minLevel: 16,
    suggestedPlayers: 3,
  },
  q_obs_brutok: {
    id: 'q_obs_brutok', name: 'The Beacon-Bull',
    giverNpcId: 'astronomer_veyra', turnInNpcId: 'astronomer_veyra',
    ...
    completionText: "Brutok's beacon is cold ash. With the last great fire out, the northern ridge has gone dark to the eye — and the faint outer stars I have been hunting for a season are finally on my plates.",
    objectives: [{ type: 'kill', targetMobId: 'brutok_skullsmasher', count: 1, label: 'Brutok Skullsmasher slain' }],
    xpReward: 1800, copperReward: 720, itemRewards: {},
    requiresQuest: 'q_obs_crushers',
    minLevel: 17,
  },
  q_obs_wyrmwatchers: {
    id: 'q_obs_wyrmwatchers', name: 'They Watch It Too',
    giverNpcId: 'astronomer_veyra', turnInNpcId: 'astronomer_veyra',
    text: "I am not the only one charting the wyrm-star, $N. The Wyrmcult zealots have raised watch-fires at the Sanctum and stand staring up at the same point of light I do — and where I see a riddle, they see a summons. Whatever that star means to wake, they mean to help it. Cut twelve of the zealots down at their fires; I will not share my sky with men who pray to it.",
    completionText: "Twelve fewer eyes turned upward in worship. They were timing their chants to the star's transit, $N — the same transit I measured. We are reading the same clock. The difference is what we each intend to do when it strikes.",
    objectives: [{ type: 'kill', targetMobId: 'wyrmcult_zealot', count: 12, label: 'Wyrmcult Zealot slain' }],
    xpReward: 2000, copperReward: 820, itemRewards: {},
    requiresQuest: 'q_obs_stormglare',
    minLevel: 17,
  },
  q_obs_deadstar: {
    id: 'q_obs_deadstar', name: 'Under the Wyrm-Star',
    giverNpcId: 'astronomer_veyra', turnInNpcId: 'astronomer_veyra',
    ...
    completionText: "Ten down, and the rest have stilled for now. Here — take my completed star-chart to the Captain; it shows when the thing falls and where. You gave me the dark to read the sky, $N, and the sky has told us we have very little time. See that the wall is ready.",
    objectives: [{ type: 'kill', targetMobId: 'boneclad_revenant', count: 10, label: 'Boneclad Revenant destroyed' }],
    xpReward: 2400, copperReward: 1050, itemRewards: {},
    requiresQuest: 'q_obs_wyrmwatchers',
    minLevel: 18,
    suggestedPlayers: 3,
  },
};
