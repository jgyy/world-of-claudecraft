...
export const DEEDS: Record<string, DeedDef> = {
  ...
  // hook") pulled Fiesta out of the queueable bracket list, so a 2v2 Fiesta
  // bout can no longer be entered by a real player and none of these seven
  // can be newly earned. Off the pvp_ prefix on purpose, the col_reliquary_complete
  ...
  // drops to 0 and they exit Book completion (deedDisplayCategory routes them
  // to the Feats shelf), which is what stops a new player's maximum from
  // sitting below a veteran's simply because this bracket is gone; earned
  // copies stay exactly as earned. Each desc states the retirement directly,
  // the feat_brightwood_relic precedent, since a stuck-looking Fiesta deed in
  // the PvP and Sport tab is what got reported as "the mode was removed."
  pvp_fiesta_first_bout: {
    id: 'pvp_fiesta_first_bout',
    name: 'Party Crasher',
    desc: 'Fight a full 2v2 Fiesta bout, win or lose. Fiesta bouts are no longer queueable, so this can no longer be newly earned.',
    category: 'pvp',
    renown: 0,
    trigger: { kind: 'manual' },
    feat: true,
  ...
  pvp_fiesta_first_win: {
    ...
    desc: 'Win a 2v2 Fiesta bout. Fiesta bouts are no longer queueable, so this can no longer be newly earned.',
    category: 'pvp',
    renown: 0,
    trigger: { kind: 'manual' },
    feat: true,
  ...
  pvp_fiesta_double: {
    ...
    name: 'Double Trouble',
    desc: 'Score two Fiesta takedowns within four seconds. Fiesta bouts are no longer queueable, so this can no longer be newly earned.',
    category: 'pvp',
    renown: 0,
    trigger: { kind: 'manual' },
    feat: true,
  ...
  pvp_fiesta_shutdown: {
    ...
    name: 'Party Pooper',
    desc: 'Take down a Fiesta foe who is on a streak of three or more. Fiesta bouts are no longer queueable, so this can no longer be newly earned.',
    category: 'pvp',
    renown: 0,
    trigger: { kind: 'manual' },
    feat: true,
  ...
  pvp_fiesta_full_build: {
    ...
    desc: 'Win a Fiesta bout with an augment locked in from all three waves. Fiesta bouts are no longer queueable, so this can no longer be newly earned.',
    category: 'pvp',
    renown: 0,
    trigger: { kind: 'manual' },
    feat: true,
  ...
  pvp_fiesta_powerups: {
    ...
    desc: 'Grab each of the four ring power-ups at least once: Speed Demon, Colossus, Moon Boots, and Berserker. Fiesta bouts are no longer queueable, so this can no longer be newly earned.',
    category: 'pvp',
    renown: 0,
    trigger: {
      kind: 'visits',
  ...
  pvp_fiesta_five_kills: {
    ...
    desc: 'Score five takedowns in a single Fiesta bout. Fiesta bouts are no longer queueable, so this can no longer be newly earned.',
    category: 'pvp',
    renown: 0,
    trigger: { kind: 'manual' },
    feat: true,
