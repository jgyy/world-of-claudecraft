...
export const DEEDS: Record<string, DeedDef> = {
  ...
  // keeps it OUT of BOOK_COMPLETE_REQUIREMENTS. Six catalog slots are
  // owner-pended today (masterwork:engineering, reins_drakemaw_raptor,
  // reins_terrorspark_groundshaker, reins_veil_wraith_courser,
  // reins_grimtusk_boar, reins_cinderhide_hound), so a non-feat capstone
  // would dead-end The Whole Book for every player (the retroFallbackGrants
  // stranded-heal doctrine names exactly that failure). It stays on the
  // Collection shelf beside its ladder; grant, marquee, and feed behavior are
  // unaffected.
