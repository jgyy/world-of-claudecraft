...
| `progression/honing.ts` + `honing_policy.ts` + `honing_record.ts` | Honing: virtual levels (post-cap lifetime-XP levels) spent as an enchanting resource on WORN gear (`resolveHoningAttempt`, one draw per resolved attempt; `honingInfoFor`); the tuning leaf (cost ladder, chance curve, `HONING_FAIL_RESETS`, glow tiers, `unspentVirtualLevels` over `PlayerMeta.virtualLevelsSpent`); the per-copy `honing` record's load-bound shape. The record is a SEPARATE additive channel item_instance_stats.ts folds in, never a rolled.stats writer |
