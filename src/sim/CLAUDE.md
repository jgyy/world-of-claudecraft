...
| `waystones.ts` + `waystone_fee.ts` + `waystone_tickets.ts` | waystones (teleport-style fast travel): the keeper spawn on RESERVED ids (`spawnWaystoneKeepers`, the Warfare Quartermaster precedent, rng-free), attunement on interact (`attuneWaystone`, persisted as `CharacterState.waystonesAttuned`) and the instant paid hop (`waystoneTeleport`: reach, attuned stone, ticket-or-fee, then `displacement.ts`). `waystone_fee.ts` is the pure fee leaf both the sim and the HUD quote from (distance-priced, guild-tier discount); `waystone_tickets.ts` is the ONE ticket-grant seam (`grantWaystoneTickets`) plus the daily Dungeon Finder clear hook (`awardFinderClearTickets`, called beside `awardHeroicMarks` from the death hub; the finder stamps `PlayerMeta.finderRunDungeonId` on formation). Data in `content/waystones.ts`; pinned by `tests/waystones.test.ts` and `tests/waystone_tickets.test.ts` |
...
| `party_gate.ts` | the mage Grand Portal and the warlock Hellgate: temporary party-gated travel objects on the Soulwell doctrine (eligibility captured at cast, late joiners via the party hook); a portal moves an eligible clicker to its destination, a gate pulls the owner's targeted ally to it, and `updatePartyGates` ends a Hellgate (and its toll aura) with its warlock |
| `grand_teleport_learning.ts` | how a mage learns a Grand Teleport: the tome use arm (`learnSpell` items) and the clear-every-city-quest unlock (`cityQuestIds`, a memoized pure derivation over the hub disc); both record the learn key in questsDone |
