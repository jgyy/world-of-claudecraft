...
| `reliquary_account.ts` + `reliquary_reads.ts` | the account-bound half of the Reliquary. `reliquary_account.ts` is the pure account ledger (`normalizeAccountReliquaryLedger`, the catalog-bounded decode every host runs on stored rows and wire payloads; `accountReliquaryLedgerFromOwnership`; `mergeAccountReliquaryLedger`, whose `grew` is the ONLY persist/re-broadcast trigger). THE union lives in `reliquary.ts` (`withAccountRelics`, reached through `reliquaryOwnershipOpts` for the facet reads and `characterReliquaryOwnership` for every grant path) and reads `PlayerMeta.accountRelics`, a live-only server stamp handed in through `addPlayer` opts and never serialized. `reliquary_reads.ts` is the one implementation of the four `IWorldReliquary` completion reads both hosts delegate to. Draw NO rng |
