...
| `party_gate.ts` | the mage Grand Portal and the warlock Hellgate: temporary party-gated travel objects on the Soulwell doctrine (eligibility captured at cast, late joiners via the party hook `rememberPartyGateEligibility`); a portal moves an eligible clicker to its authored destination (`content/grand_teleports.ts`), a gate pulls the owner's targeted ally to it, and `updatePartyGates` (the tick pass after despawn decay) ends a Hellgate and its toll aura with its warlock; draws NO rng. `line_of_sight_gate.ts` is the sibling extraction that pays for its coordinator wiring (the delve-aware `hasLineOfSight` and the composed `lineOfSightBlocked`, over `ability_line_of_sight.ts`) |
