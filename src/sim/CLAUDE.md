...
| `flight_paths.ts` | flight paths: the flightmaster spawn on RESERVED ids (`spawnFlightmasters`, the Warfare Quartermaster precedent, rng-free), discovery on interact (`discoverFlightmaster`, persisted as `CharacterState.flightNodesKnown`), the paid BFS-routed boarding gate (`takeFlight`: reach, known node, route, fare) and the hands-off ride MOVEMENT MODE (`advanceFlightPath`, same family as `climb.ts` and Heroic Leap: owns the body until it lands through `teleport_arrival.ts`). Data in `content/flight_paths.ts`; pinned by `tests/flight_paths.test.ts` |
