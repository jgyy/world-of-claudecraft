// The Rift forge trio's wire dispatch (rift_upgrade_item / rift_enchant_item /
// rift_socket_gem), lifted out of the GameServer command switch so the arms
// share one parse and one ack contract.
//
...
function bagSlot(msg: ForgeMessage): number | undefined {
  return Number.isInteger(msg.slot) ? Number(msg.slot) : undefined;
}

/** Run one forge command against the sim; null when the frame is malformed
 *  or names a non-forge token. */
export function dispatchRiftForgeCommand(
  sim: Pick<Sim, 'upgradeRiftItem' | 'enchantRiftItem' | 'socketRiftGem'>,
  msg: ForgeMessage,
  pid: number,
  ...
  switch (msg.cmd) {
    case 'rift_upgrade_item':
      return sim.upgradeRiftItem(msg.item, pid, bagSlot(msg));
    case 'rift_enchant_item':
      return typeof msg.stat === 'string'
        ? sim.enchantRiftItem(msg.item, msg.stat, pid, bagSlot(msg))
        : null;
    case 'rift_socket_gem':
      return typeof msg.gem === 'string'
        ? sim.socketRiftGem(msg.item, msg.gem, pid, bagSlot(msg))
        : null;
    default:
      return null;
  }
